...
training_data['bot'] = y_train
header = cols + ['bot']
testing_data = X_test
testing_data['bot'] = y_test
...
def print_leaf(counts):
    ...
    return probs
##################################################################################################
def prob_leaf(counts):
    """return the predictions at a leaf."""
    if len(counts)<2:
...
error = 0
for row in testing_data:
    prob_dict = prob_leaf(classify(row, my_tree))
    # bot
    if row[-1] == 1:
...
print('The accuary is {}'.format(1-error/len(testing_data)))
